...
static inline int union_find(int x, int* table)
{
...
void print_tour(int tour[], struct point pts[], int n_pts)
{
    int i;
    for (i = 0; i < n_pts; i++) {
        struct point* p = pts + tour[i];
        printf("%4d %6d %6d\n", p->index, p->x, p->y);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    int lst_tour[MAX_N];
    build_lst_path(pts, n_pts, lst_tour);
    //print_tour(lst_tour, pts, n_pts);
    putchar('\n');

    for (afactor = 0.0; afactor < 5.05; afactor += 5.0) {
    for (mfactor = 1.5; mfactor > 1.05; mfactor -= 0.1) {
        memcpy(tour, lst_tour, sizeof(int) * n_pts);
        bool success;
        int count = 0;
        do {
            success = false;
    ...
    for (mfactor = 1.5; mfactor > 1.0; mfactor -= 0.1) {
        ...
        //shape_map(pts, n_pts, shaped_pts, 100, 0.6);
